 ...
 */
(function(undefined) {
  var EgretGraphics = egret.Graphics,
   ...
   * @class Graphics
   * @extends egret.Graphics
   * @constructor
   */
  var Graphics = function()
  {
    EgretGraphics.call(this);
    DisplayObject.call(this);
  };

  // Extend egret.Sprite
  var s = EgretGraphics.prototype;
  ...
  // Assign to namespace
  egretAnimate.Graphics = Graphics;
   ...
   * @return {egretAnimate.Graphics} The Graphics instance the method is called on (useful for chaining calls).
   **/
  p.mt = p.moveTo;
  ...
  /**
   * Shortcut to drawRoundRectComplex. Not supported by egretAnimate
   * @method rc
   * @param {Number} x
   * @param {Number} y
   * @param {Number} w Width of the rectangle
  ...
  p.qt = function(cpx, cpy, x, y)
  {
    ...
    return this.op().curveTo(cpx, cpy, x, y);
  };
  ...
  p.lf = function(colors, ratios, x0, y0, x1, y1)
  {
    ...
    // https://github.com/egret-labs/egret-core/issues/139
    //
    var matrix = new egret.Matrix(1, x0 / 819.2, 1, y0 / 819.2);

    this.beginGradientFill(
      egret.GradientType.LINEAR,
      colors,
      alphas,
      ratios.map(function(ratio) { return ratio * 255 }),
      matrix
    );
  ...
  p.ss = function(thickness, caps, joints, miterLimit, ignoreScale)
  {
    ...
    this.lineStyle(this.lineWidth, this.lineColor, this.lineAlpha, ignoreScale, "normal", caps, joints, miterLimit);
